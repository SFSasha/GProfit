...
FIXED_TASKS = [
    {"id": -2, "url": "https://t.me/+jZY37XZ12Cw0NjZi", "check_id": -1002996957244, "stars": 10},
]
...
async def format_weekly_referral_top(top_list: List[Tuple[int, int]]) -> str:
    ...
    top_text = "*🏆 ТОП-5 Рефералов за Неделю *\n\n"
...
def get_back_to_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    back_button = InlineKeyboardButton(text="⬅️", callback_data="back_to_menu") 
    return InlineKeyboardMarkup(inline_keyboard=[[back_button]])

async def format_weekly_referral_top(top_list: List[Tuple[int, int]], bot: Bot) -> str:
    """Форматирует список топа еженедельных рефералов, показывая только полное имя."""
    
    # Используем MarkdownV1 (со звездочками *)
    top_text = "*🏆 Топ-10 по приглашениям за неделю*\n\n"
    
    if not top_list:
        # Используем MarkdownV1 для болда
        return "😔 *Еженедельный топ рефералов* пока пуст. Будьте первым!"

    
    for i, (user_id, count) in enumerate(top_list):
        # 💡 Инициализируем заглушку, используя MarkdownV1 болд
        user_display_name = "*[Имя не указано]*" 
        
        try:
            # Используем await bot.get_chat для получения данных
            chat = await bot.get_chat(user_id)
            
            # ✅ Приоритет: Полное имя (Full Name)
            if chat.full_name:
                user_display_name = chat.full_name
            # Игнорируем chat.username, чтобы показывалось только имя
            ...
            user_display_name = "*[Недоступен]*" # Заглушка для ошибок
        ...
        # Используем MarkdownV1 для болда
        top_text += f"{emoji} *{user_display_name}* - {count} человек\n"
