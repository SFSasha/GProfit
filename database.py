...
async def safe_edit(message, text=None, reply_markup=None):
    ...
    Просто пробуем edit_text, игнорируя BadRequest и ошибки 'message is not modified'.
    """
    if message is None:
        return
    ...
    try:
        ...
        if "message is not modified" in str(e):
            ...
            logging.warning(f"Safe edit failed (TelegramBadRequest): {e}")
    except Exception as e:
        logging.warning(f"Safe edit failed (Other): {e}")
# --- FSM STATES ---
...
@router.callback_query(F.data.startswith("skip_flyer_"))
async def skip_flyer_task(callback: types.CallbackQuery, bot: Bot):
    """Пропуск текущего задания Flyer и показ следующего."""
    signature_to_skip = callback.data.split("skip_flyer_")[-1]

    # Просто логируем и передаем skip в flyer_check_wrapper
    logging.info(f"User {callback.from_user.id} skipped Flyer task {signature_to_skip}")

    # Показываем следующее задание
    await earn_stars_start(callback, bot, skip_signature=signature_to_skip)
...
async def get_flyer_tasks(user_id: int) -> Optional[Dict]:
    """Получает одну задачу от Flyer API."""
    if not FLYER_API_KEY:  # <-- проверка наличия ключа
        ...
        try:
            response = await client.post(
                ...
                json={
                    "key": FLYER_API_KEY,
                    "user_id": user_id,
                    "limit": 1 # Запрашиваем одну задачу
                }
            )
            ...
            # Если есть результат и в нем есть задания
            if data.get('result') and isinstance(data['result'], list) and data['result']:
                return data['result'][0]
                
        except httpx.HTTPStatusError as e:
            logging.error(f"Flyer API HTTP Error: {e.response.status_code} - {e.response.text}")
        except httpx.RequestError as e:
            logging.error(f"Flyer API Request Error: {e}")
        except Exception as e:
            logging.error(f"Flyer API Unknown Error: {e}")
    return None


async def check_flyer_task(user_id: int, signature: str) -> Dict:
    """Проверяет выполнение задачи Flyer."""
    if not FLYER_API_KEY:  # <-- проверка наличия ключа
...
async def flyer_check_wrapper(user: types.User, message: types.Message, action: str = "subscribe") -> Dict:
    ...
    if task_data:
        # Успешно получили задачу Flyer
        reward = FLYER_TASK_REWARD # 1 звезда за задачу Flyer
        task_signature = task_data['signature']
        
        # --- ИСПРАВЛЕНИЕ: БЕЗОПАСНОЕ ПОЛУЧЕНИЕ ССЫЛКИ ---
        # Проверяем 'url', а затем 'link', т.к. API может возвращать одно из них.
        task_url = task_data.get('url') or task_data.get('link')

        if not task_url:
            logging.error(f"Flyer API task {task_signature} is missing 'url' and 'link'. Skipping.")
        ...
        # Отображение задания Flyer
        await safe_edit(message, 
            text="💎 **СРОЧНОЕ ЗАДАНИЕ (Flyer)**\n\n"
...
# ----------------- ОБНОВЛЕННЫЙ EARN TASK -----------------

@router.callback_query(F.data == "earn_task_menu")
async def earn_stars_start(callback: types.CallbackQuery, bot: Bot):
    """Отображение задания для заработка: Flyer -> пользовательские задачи."""
    user_id = callback.from_user.id
    user = callback.from_user
    message = callback.message
    logging.info(f"EARN TASK triggered for user {user_id}")

    # --- 1. Проверяем задания Flyer ---
    subgram_data = await flyer_check_wrapper(user=user, message=message, action="subscribe")
    
    if subgram_data and not subgram_data.get("skip"):
        await callback.answer()
        return  # Flyer задание показано, дальше не идем

    # --- 2. Проверяем пользовательские задания ---
    ...
    if task:
        ...
        await safe_edit(message,
            text=f"💰 **ЗАДАНИЕ: ПОДПИСКА НА КАНАЛ**\n\n"
    ...
    await safe_edit(message,
        text="😔 **ЗАДАНИЯ ЗАКОНЧИЛИСЬ**\n\n"
             "На данный момент нет доступных заданий ни от Flyer, ни от пользователей. Попробуйте зайти позже.",
        reply_markup=kb
    )
    await callback.answer()
...
async def check_flyer_task_done(callback: types.CallbackQuery, bot: Bot):
    """Проверка выполнения задания Flyer и начисление награды."""
    # Получаем сигнатуру (ID) задания
    ...
    if result['success']:
        ...
        await callback.answer("🔄 Задание ожидает подтверждения. Попробуйте проверить еще раз через некоторое время.", show_alert=True)
        # Просто остаемся на текущем меню, чтобы пользователь мог проверить еще раз
        await earn_stars_start(callback, bot)
        return
        
    elif result.get('result') == 'key_error' or result.get('result') == 'api_error':
        # Не удалось проверить из-за ошибки ключа/API
        await callback.answer("❌ Произошла ошибка API при проверке задания. Пожалуйста, сообщите администратору.", show_alert=True)
        
    else:
        # Failed или Internal_error
        await callback.answer("❌ Задание не выполнено или истек срок (1 час). Пожалуйста, убедитесь, что вы подписаны.", show_alert=True)
